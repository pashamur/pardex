...
module Pardex
  class Table
    attr_accessor :name, :statistics, :rowcount, :attributes
    ...
    def initialize(model)
      ...
      row_result = model.connection.exec_query(row_number_query)
      ...
      self.attributes = statistics.map do |stats|
        ...
        {name => Attribute.new(stats, rowcount, ar_column_type)}
      end.inject(&:merge!)
    end
